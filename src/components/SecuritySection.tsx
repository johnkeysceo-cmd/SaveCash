      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-purple-400/30 mb-6"
          >
        ...
        >
          {securityFeatures.map((feature, index) => (
            <motion.div
              key={feature.title}
              variants={item}
              className="group relative p-8 rounded-3xl bg-white/5 backdrop-blur-xl border border-white/10 hover:border-purple-400/30 transition-all duration-300"
            >
              {/* Gradient glow on hover */}
              <div className={`absolute inset-0 rounded-3xl bg-gradient-to-br ${feature.bgColor} opacity-0 group-hover:opacity-100 transition-opacity duration-300 blur-xl -z-10`} />
              
              <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${feature.bgColor} flex items-center justify-center mb-6 border border-white/10 group-hover:border-purple-400/30 transition-all duration-300 relative`}>
                <feature.icon className={`w-7 h-7 text-purple-400`} />
              </div>
              
              <h3 className="text-2xl text-white mb-4 font-semibold group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-purple-400 group-hover:to-pink-400 group-hover:bg-clip-text transition-all duration-300">
                {feature.title}
              </h3>
        ...
        >
          <motion.h3
            className="text-3xl md:text-4xl text-white text-center mb-4 font-semibold"
            initial={{ opacity: 0 }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 mb-6">
            {complianceBadges.map((badge, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="group p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 hover:border-purple-400/30 transition-all duration-300 text-center"
              >
                <BadgeCheck className="w-8 h-8 text-purple-400 mx-auto mb-3 group-hover:scale-110 transition-transform duration-300" />
                <div className="text-white font-semibold mb-1 text-sm">
                  {badge.name}
                  {badge.note && <span className="text-gray-500 font-normal ml-1">{badge.note}</span>}
                </div>
          ...
          <motion.p
            className="text-center text-gray-400 text-sm max-w-2xl mx-auto font-medium"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
        ...
        <motion.div
          ...
          transition={{ duration: 0.8, ease: [0.25, 0.1, 0.25, 1] }}
          viewport={{ once: true, margin: "-100px" }}
          className="max-w-4xl mx-auto p-10 rounded-3xl bg-gradient-to-br from-purple-500/10 via-pink-500/10 to-purple-500/10 border border-purple-400/20 backdrop-blur-xl"
        >
          <div className="flex items-start gap-6">
            <div className="flex-shrink-0 w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-purple-400/30 flex items-center justify-center">
